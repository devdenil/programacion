...
const ManagerDistributionsCreate = ({ showSuccessAlert, showErrorAlert, showLoading }) => {
    ...
    const handleAddToCart = (productId, availableQuantity) => {
        ...
        if (existingCartItem) {
            ...
            setCart(prevCart => [...prevCart, { productId, quantity: 1, price }]);
    ...
    const handleCompletePurchase = async () => {
        ...
        try {
            await Promise.all(cart.map(async item => {
                ...
                // Check if a distribution already exists for this product and user
                const distributionsRef = firebase.firestore().collection('distributions');
                const existingDistributions = await distributionsRef
                    .where('productId', '==', item.productId)
                    .where('sellerId', '==', selectedUser || 'Seller')
                    .get();
    
                if (!existingDistributions.empty) {
                    // Distribution exists, update it
                    existingDistributions.forEach(async doc => {
                        const existingData = doc.data();
                        const newQuantity = existingData.productquantity + item.quantity;
                        await doc.ref.update({ productquantity: newQuantity });
                    });
                } else {
                    // No distribution exists, create a new one
                    const distributionData = {
                        sellerId: selectedUser || 'Seller',
                    ...
                    await distributionsRef.add(distributionData);
                }
            }));
            ...
            showSuccessAlert('Entrega Registrada Exitosamente');
        } catch (error) {
            ...
            <div className="row">
                ...
                <div className="col-md-4">
                    ...
                    <button className="btn mb-4" onClick={handleCompletePurchase} disabled={loading}>
                        {loading ? 'Cargando...' : 'Completar Entrega'}
                    </button>
                </div>
            </div>
        </div>
...
export default ManagerDistributionsCreate;